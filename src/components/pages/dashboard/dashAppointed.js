...
class dashAppointed extends Component {
  constructor () {
    super()
    this.state = {
    ...
    return (
      <ScrollView style={styles.container} contentContainerStyle={{ alignItems: 'center', paddingTop: 35,}}>
          {dash.reverse().map((data, index) => {
            if (data.state == 1) {
              return (
                <DashAppointCell date={data.date} add={data.add} process={data.process} title={data.title} navigation={this.props.navigation} link='error' key={index} width={width} />
              )
            }
          })}
      </ScrollView>
